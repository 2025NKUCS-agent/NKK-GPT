...
from typing import Any, Callable, List, Literal, Tuple, Union, TYPE_CHECKING
...
from nkkagent.const import MESSAGE_ROUTE_TO_ALL
...
def is_send_to(message: Any, addresses: set):
    """Check if the message is sent to any of the addresses."""
    if not message or not hasattr(message, "send_to"):
        return False
    if not addresses:
        return False
...
def encode_image(image_path_or_pil: Union[Path, 'PILImage'], encoding: str = "utf-8") -> str:
