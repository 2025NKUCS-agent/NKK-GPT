...
from nkkagent.const import DEFAULT_WORKSPACE_ROOT 
...
def _convert_path_relative_to_repo_root(path: Path | str, root: Path | None = None) -> Path | str:
    ...
    root = Path(root or os.getenv("SWE_AGENT_CONFIG_ROOT", DEFAULT_WORKSPACE_ROOT ))
...
def _convert_path_to_abspath(path: Path | str) -> Path:
    ...
    DEFAULT_WORKSPACE_ROOT  as base.
    """
    path = Path(path)
    root = Path(os.getenv("SWE_AGENT_CONFIG_ROOT", DEFAULT_WORKSPACE_ROOT ))
    assert root.is_dir()
...
def load_environment_variables(path: Path | None = None):
    ...
    if path is None:
        ...
        repo_path = DEFAULT_WORKSPACE_ROOT  / ".env"
        if cwd_path.exists():
            path = cwd_path
        elif repo_path.exists():
            path = DEFAULT_WORKSPACE_ROOT  / ".env"
        else:
